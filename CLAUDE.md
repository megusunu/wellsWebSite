...
### v2.7.0 - 技術書記事とイベント記事の追加・更新

1. **技術書同人誌博覧会12記事の更新**
   - 2025年10月26日開催イベントの記事作成
   - Square購入リンク追加（電子版、物理本、セット）
   - 「QRコードを編む」と「推しキャラを召喚！LINE BOT」の2冊
   - 本書のメッセージセクション追加（序文・あとがきベース）
   - 著者を「草間 恵」に変更

2. **竹芝「Bamboo Grass Market」出店記事**
   - 2025年11月1日開催イベントの詳細記事
   - ブランド名を「megusunuLab」に統一（小文字m）
   - ペット用・人用スヌードの「隠れたおそろコーデ」を詳しく紹介
   - イベントの雰囲気（ノルウェー音楽、デカスロン体験など）追加
   - お客様の反応と新しい使い方（ペット用腹巻き）の発見
   - 次回イベント（11月3日中野セントラルパーク）の予告
   - スヌード写真3枚をグリッドレイアウトで追加
   - 連絡先セクションの整理（オンライン販売、SNS、お問い合わせを分離）

3. **三田マルシェ出店記事の追加**
   - 2025年10月25日開催のハロウィンイベント記事
   - かぼちゃの編みぐるみワークショップ（15〜20分）
   - イベントポスター画像とかぼちゃ写真3枚を追加
   - 三田マルシェの特徴（やってみたマルシェ）を紹介
   - ペット用ハロウィンアイテム（ケープ、帽子）の販売報告

4. **画像レイアウトの改善**
   - 記事画像サイズを最適化（max-width: 600px）
   - 複数画像用のグリッドレイアウト実装
   - レスポンシブ対応（モバイルで1列、デスクトップで2〜3列）
   - 以下の記事に適用：
     - 竹芝マーケット（ブース・商品・スヌード）
     - 根津神社（ブース2枚）
     - 阿波井フェスティバル（商品3枚）
...
5. **価格表示の方針変更**
   - イベントごとに価格が変動するため、記事から具体的な価格を削除
   - 根津神社記事：「販売予定作品と価格」→「販売作品」に変更
   - 価格確認の注釈も削除（シンプルな表現に）

6. **画像アセット追加**
   - `/public/images/events/mita-marche/` - イベントポスター、かぼちゃ写真3枚
   - `/public/images/events/takeshiba-market/` - スヌード写真3枚

### v2.8.0 - おもしろ同人誌バザール記事追加

1. **おもしろ同人誌バザール出展記事**
   - 2025年11月2日開催の神保町イベント記事
   - 技術書2冊の詳細な紹介
     - 推しキャラを召喚！LINE BOTで実現する日常（LINE Messaging API、GAS、Gemini AI）
     - QRコードを編む（QRコードの技術解析と手編み実験）
   - 手編みQRコードと手編みグッズの展示
   - 来場者の反応（手編みQRコードへの関心、手編みグッズの可能性）
   - イベント写真3枚（看板、ブース全体、手編みQRコード）
   - 公式サイトリンク追加（https://omobaza.com/）

2. **記事のトーン調整**
   - カジュアルで等身大の雰囲気に変更
   - 仰々しい表現を削除し、親しみやすい文体に
   - 体験談ベースの自然な流れ
   - 「持って行ったもの」「次回に向けて」など柔らかい見出し

3. **技術書説明の充実**
   - 各技術書の使用技術と目的を明記
   - 実験的なアプローチ（QRコードの歪み耐性）を強調
   - 完全無料で運用できることを明記

4. **画像アセット追加**
   - `/public/images/events/omoshiro-doujinshi-bazaar/` - イベント看板、ブース全体、手編みQRコード

### v2.9.0 - SEO基本設定実装

1. **サイトマップ自動生成**
   - `@astrojs/sitemap`パッケージを導入
   - ビルド時に自動でsitemap.xmlを生成
   - Google Search Consoleへの登録を推奨
...
3. **構造化データ（JSON-LD）追加**
   - **Organization（会社情報）**: 全ページに適用
     - 会社名、設立日、代表者名、所在地、SNS、ロゴ
   - **Article（記事情報）**: 個別記事ページに適用
     - 記事タイトル、説明、公開日、著者、発行者
   - **BreadcrumbList（パンくずリスト）**: 個別記事ページに適用
     - ホーム → お知らせ → 記事

4. **実装前のデータ分析（2025年10月5日〜11月1日）**
   - トップページへのアクセスが最も多い
   - 技術書関連記事（技術書博覧会12）への関心が高い
   - **Organic Searchの比率が20%未満** → 改善の余地あり
   - **「エンジニアが編み物する理由」記事の滞在時間が長い**（読者の共感を得ている）
   - SNS（Organic Social）からの流入が一定数ある（30%程度）
   - Google検索からの流入の滞在時間が長い（質の高いトラフィック）

5. **期待される効果**
   - Organic Searchの増加（目標: 2〜3倍）
   - Google検索結果での視認性向上（リッチスニペット表示）
   - SEO評価の向上
   - 記事がGoogle検索に表示されやすくなる

6. **検証予定**
   - **1ヶ月後（2025年12月初旬）**: 効果測定
     - Organic Searchの比率変化
     - 全体のセッション数の変化
   - **2ヶ月後（2026年1月初旬）**: 追加分析
     - 長期的なトレンドの確認
...
7. **今後の改善案（優先度順）**
   - 「エンジニアが編み物」系の記事を増やす（滞在時間が長いため）
   - トップページの導線改善
   - 技術書の詳細ページ作成

---

### v2.10.0 - カテゴリーフィルタ機能追加

1. **ニュース一覧ページにカテゴリーフィルタ追加**
   - [すべて] [ニュース] [コラム] [プロジェクト] [イベント] [アップデート]
   - クリックでカテゴリー別表示
   - URLパラメータで状態保持（例: `/news?category=blog`）
   - アクティブなカテゴリーはオレンジ色で強調表示

2. **ユーザー体験の向上**
   - 見たいカテゴリーの記事だけを表示可能
   - eventカテゴリが多い中、blog/projectも探しやすく
   - URLで状態共有可能（SNSシェアに便利）

3. **技術実装**
   - クライアントサイドJavaScriptでフィルタリング
   - data属性でカテゴリー情報を管理
   - URL履歴管理（ブラウザの戻る/進むで状態維持）

4. **記事は作成せず**
   - 内部機能改善のため、記事としては発信しない
   - CLAUDE.mdに開発履歴として記録のみ

---

### v2.11.0 - 1人アドベントカレンダー2025完走

1. **アドベントカレンダー全25日分のリンク追加**
   - 12/1〜25の全記事をZennで公開完了
   - Wells HPのアドベントカレンダーページに全リンクを追加
   - 全セルが`active`状態に（`pending`なし）

2. **アドベントカレンダー記事一覧（全25日）**
   | 日 | テーマ |
   |----|--------|
   | 1 | ハンドメイドと心のセーフティネット |
   | 2 | ストレングスファインダーとAI時代 |
   | 3 | Cookie 4096バイト制限とnginx |
   | 4 | ペット用ニットウェア |
   | 5 | AIコーチングとひとり会社 |
   | 6 | 技術同人誌×特殊印刷 |
   | 7 | ハンドメイド副業の収支 |
   | 8 | ECS Fargate×Quarkusスケーリング |
   | 9 | Leafletコンビニ分布マップ |
   | 10 | WordPress×ChatGPT API |
   | 11 | Claude Codeカスタムコマンド |
   | 12 | アドベントカレンダー折り返し |
   | 13 | 3Dプリンター購入体験 |
   | 14 | アクリルたわし200個 |
   | 15 | DynamoDB設計の失敗 |
   | 16 | DynamoDB暗号化とKMS |
   | 17 | DynamoDB begins_withの罠 |
   | 18 | 子供向けワークショップ |
   | 19 | AWS CloudShell 30分制限 |
   | 20 | AIで会社HP半年運用 |
   | 21 | UPSIDER資金管理 |
   | 22 | LINE BOT同人誌改訂 |
   | 23 | Squareクリスマス仕様 |
